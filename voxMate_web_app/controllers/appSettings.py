...
def settings():
    ...
    # Checks if the The form validates, and The form data contains the key
    # 'noise_reduction' & 'volume_display
    #  — regardless of whether its value is True or False, just that the key exists.
    # Checking noise_reduction here as WTForms does not handle boolean
    # field validation well
    if settingsForm.validate_on_submit() and all(
        key in settingsForm.data for key in ["noise_reduction", "volume_display"]
    ):

        # Collect the form data, excluding the CSRF token & submit fields
        form_data = {
            k: v
            for k, v in settingsForm.data.items()
        ...
        settingUpdate = current_app.db.appSettings.update_one(
            {"user_id": session["user_id"]}, {"$set": form_data}
        )
    ...
    return render_template(
        "appSettings/settings.html",
        title="voxMate - Settings",
        settings=settings,
        settingsForm=settingsForm,
