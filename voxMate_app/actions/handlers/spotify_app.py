...
class SpotifyPlayer:
    ...
    def create_spotify_oauth(self) -> Optional[SpotifyOAuth]:
        ...
        try:
            if (mongodb := load_mongodb()) is None:
                return None
            state = mongodb.users.find_one({"user_id": self.user_id}).get(
                "api_token", ""
            )
            return SpotifyOAuth(
    ...
    def load_spotify_token(self) -> Optional[dict]:
        ...
        try:
            ...
            if current_time > expires_at - 300:
                ...
                if not (
                    new_token_info := oauth.refresh_access_token(
                        doc.token_info["refresh_token"]
                    )
                ):
    ...
    def _match_spotify_item(
        self, query: str, items: list, content_type: str
        ...
        for item in items:
            ...
            if name == query_lower:
                logger.info(
                    f"Exact match: '{query}' in '{name}', type '{content_type}'"
                )
            ...
            if ratio(query_lower, name) > 80:
                logger.info(
                    f"Fuzzy match ({ratio(query_lower, name)}%): '{query}' vs '{name}'"
                )
                return content_type, item["uri"]
        return None

    def detect_spotify_type(
        ...
        q = (
            f"track:{query_lower} artist:{artist_lower}"
            if artist_lower
            else query_lower
        )
    ...
    def _verify_playback_transfer(self, device_id: str, timeout: int = 5) -> bool:
        ...
        while time.time() < start_time + timeout:
            try:
                if (current := self.sp.current_playback()) and current.get(
                    "device", {}
                ).get("id") == device_id:
                    return True
                time.sleep(1)
        ...
        try:
            ...
            if query == "news":
                ...
                try:
                    ...
                    return (
                        self._handle_spotify_error("news podcast playback", e),
                        "Error getting news podcast",
                    )

            # Handle playback with query
            content_type, uri = self.detect_spotify_type(
                query, artist, user_content_type
            )
            if not uri:
                return self._start_fallback_playback(device_id)
    ...
    def stop_playback(self) -> Tuple[bool, Optional[str]]:
        return self._basic_playback_control(
            "pause_playback", error_msg="Failed to stop playback"
        )

    def skip_playback(self) -> Tuple[bool, Optional[str]]:
        return self._basic_playback_control(
            "next_track", error_msg="Failed to skip track"
        )
    ...
    def repeat_playback(self, repeat) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            return self._handle_spotify_error(
                "repeat toggle", e, "Failed to toggle repeat mode"
            )
    ...
    def shuffle_playback(self, shuffle) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            return self._handle_spotify_error(
                "shuffle toggle", e, "Failed to toggle shuffle mode"
            )
        except Exception as e:
            return self._handle_spotify_error(
                "shuffle toggle", e, "Failed to toggle shuffle mode"
    ...
    def save_spotify_doc(self, doc: VoxSpotify) -> bool:
        ...
        try:
            ...
            mongodb.voxSpotify.update_one(
                {"user_id": self.user_id}, {"$set": doc.to_dict()}, upsert=True
            )
            return True
        except Exception as e:
