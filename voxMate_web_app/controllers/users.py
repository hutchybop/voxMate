...
def register():
    ...
    # If the form is submitted and valid, create a new user with
    # unverified_user_id and send them to register/verify
    if form.validate_on_submit():
        ...
        if os.getenv("FLASK_ENV_PI") == "true":
            ...
            if not device_id:
                flash(
                    f"Error retrieving Raspberry device ID: {error}.",
                    "Please try again.",
                    "danger",
                )
        ...
        payload = {
            "device_id": device_id,
            "unverified_user_id": user.unverified_user_id,
            "email": user.email,
        }
        ...
        if response and response.get("success"):
            ...
            session["unverified_user_id"] = (
                # Using unverified_user_id and not user_id as user not verifed
                unverified_user_id
            )
...
def verify():
    ...
    if form.validate_on_submit():
        # Get the code the user has entered, which should be sent to the email
        ...
        if response:
            if response.get("success"):
                ...
                if not api_token:
                    flash("Error connecting the server. Please try again.", "danger")
                    return redirect(url_for("users.verify"))
                # Add the unverified_user_id to user_id
                user_id = unverified_user_id
                # Change the user id from unverified to user_id and changed
                # verifiy to true in the DB
...
def get_code():
    ...
    if not user:
        # If not registered in db, remove the unverified_user_id from the
        # seesion and send to register
        session.pop("unverified_user_id", None)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # If the user's verify is false add unverified_user_id to the session
        # and redirect to verify
        if user_data.get("verify") is False:
            if user_data.get("unverified_user_id"):
                session.pop("user_id", None)
                session["unverified_user_id"] = user_data.get("unverified_user_id")
                current_app.db.users.update_one(
                    {"email": form.email.data}, {"$set": {"user_id": None}}
                )
                ...
                current_app.db.users.update_one(
                    ...
                    {
                        "$set": {"user_id": None},
                        "unverified_user_id": user_data.get("user_id"),
                    },
                )
                flash("Please verify your email address")
                return redirect(url_for("users.verify"))
